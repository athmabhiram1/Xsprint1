            ...
            for (const match of event.matches) {
                if (match.playerA &&
                    ...
                    sameClubClashes.push({
                        ...
                        clubName: ((_a = match.playerA.club) === null || _a === void 0 ? void 0 : _a.name) || 'Unknown',
                        round: match.round,
                    });
                }
            }
    ...
    getScheduleQuality(eventId) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (const court of courts) {
                const courtMatches = scheduledMatches.filter((m) => { var _a, _b; return ((_b = (_a = m.schedule) === null || _a === void 0 ? void 0 : _a.court) === null || _b === void 0 ? void 0 : _b.name) === court.name; });
                const usedSlots = courtMatches.length;
                // Calculate total possible slots based on tournament duration
                const tournamentDays = Math.ceil((tournament.endDate.getTime() - tournament.startDate.getTime()) /
                    (1000 * 60 * 60 * 24));
                const hoursPerDay = 10; // 9 AM to 7 PM
                const totalSlots = Math.floor((tournamentDays * hoursPerDay * 60) / matchDuration);
            ...
            const averageUtilization = courtUtilization.reduce((sum, c) => sum + c.utilizationPercent, 0) /
                (courtUtilization.length || 1);
            // Find rest-time issues
            const restTimeIssues = [];
            ...
            playerMatches.forEach((matches, playerId) => {
                const sortedMatches = matches.sort((a, b) => { var _a, _b; return (((_a = a.startTime) === null || _a === void 0 ? void 0 : _a.getTime()) || 0) - (((_b = b.startTime) === null || _b === void 0 ? void 0 : _b.getTime()) || 0); });
                for (let i = 0; i < sortedMatches.length - 1; i++) {
                    ...
                    if (current.startTime && next.startTime) {
                        const restMinutes = (next.startTime.getTime() -
                            current.startTime.getTime() -
                            matchDuration * 60 * 1000) /
                            (1000 * 60);
                        if (restMinutes < requiredRestMinutes) {
                            const player = current.playerAId === playerId ? current.playerA : current.playerB;
                            restTimeIssues.push({
                                playerId,
                                playerName: (player === null || player === void 0 ? void 0 : player.name) || 'Unknown',
            ...
            const backToBackMatches = restTimeIssues.filter((issue) => issue.actualRestMinutes < 15).length;
