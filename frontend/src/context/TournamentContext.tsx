...
export const TournamentProvider = ({ children }: { children: ReactNode }) => {
    ...
    const [recentActivity, setRecentActivity] = useState<Activity[]>([
        { id: 1001, text: 'System initialized', time: '2m ago', type: 'system' },
        { id: 1002, text: 'Alex Rivera registered', time: '15m ago', type: 'registration' }
    ]);
    
    // Activity ID counter to prevent duplicates
    const activityIdCounter = React.useRef(1002);
    ...
    const addActivity = (text: string, type = 'system') => {
        ...
        const newActivity = { id: activityIdCounter.current, text, time: 'Just now', type };
        setRecentActivity(prev => {
            // Prevent duplicate activities with the same text
            const exists = prev.some(activity => activity.text === text);
